...
/// Selects the value on which an error polynomial is evaluated.
#[derive(Debug, Clone, Serialize, Deserialize, Copy, PartialEq, Eq, Default)]
pub enum ErrorPolyEvaluation {
    /// Evaluate the polynomial on the measured observation (legacy assay-error behavior).
    #[default]
    Observation,
    /// Evaluate the polynomial on the model prediction.
    Prediction,
}
...
/// More simply, it is the error associated with the observed or predicted value.
/// [`ErrorPoly::new`] retains observation-based evaluation, while
/// [`ErrorPoly::prediction_based`] evaluates on the model prediction.
/// The polynomial is defined as:
///
/// ```text
/// error = c0 + c1 * value + c2 * value^2 + c3 * value^3
/// ```
///
...
pub struct ErrorPoly {
    ...
    c3: f64,
    #[serde(default)]
    evaluation: ErrorPolyEvaluation,
}
...
impl ErrorPoly {
    pub fn new(c0: f64, c1: f64, c2: f64, c3: f64) -> Self {
        Self {
            ...
            c2,
            c3,
            evaluation: ErrorPolyEvaluation::Observation,
        }
    }

    /// Construct an error polynomial evaluated on the model prediction.
    pub fn prediction_based(c0: f64, c1: f64, c2: f64, c3: f64) -> Self {
        Self {
            c0,
            c1,
            c2,
            c3,
            evaluation: ErrorPolyEvaluation::Prediction,
        }
    }
    ...
    /// Return whether the polynomial uses the observation or prediction.
    pub fn evaluation(&self) -> ErrorPolyEvaluation {
        self.evaluation
    }
...
/// from the **observation** value by default; a prediction-based [`ErrorPoly`]
/// can opt into evaluation on the model prediction.
///
...
impl AssayErrorModels {
    ...
    ///
    /// The polynomial selects whether `sigma` is evaluated on the observation
    /// (the legacy assay-error behavior) or on the model prediction.
    ///
    /// # Arguments
    ///
...
impl AssayErrorModel {
    ...
    pub fn sigma(&self, prediction: &Prediction) -> Result<f64, ErrorModelError> {
        ...
        let value = match errorpoly.evaluation {
            ErrorPolyEvaluation::Observation => prediction.observation().unwrap(),
            ErrorPolyEvaluation::Prediction => prediction.prediction(),
        };
...
mod tests {
    ...
    #[test]
    fn test_prediction_based_additive_error_model() {
        let observation = Observation::new(0.0, Some(20.0), 0, None, 0, Censor::None);
        let prediction = observation.to_prediction(10.0, vec![]);
        let model = AssayErrorModel::additive(ErrorPoly::prediction_based(1.0, 2.0, 0.0, 0.0), 5.0);
        assert_eq!(model.sigma(&prediction).unwrap(), (466.0_f64).sqrt());
        assert_eq!(
            model.errorpoly().unwrap().evaluation(),
            ErrorPolyEvaluation::Prediction
        );
    }
    ...
    #[test]
    fn test_error_models_bind_output_names() {
        let error_models = AssayErrorModels::new()
            .add(
                "effect",
